            ...
            <div className="max-h-[75vh] overflow-y-auto p-5 md:p-8 custom-scrollbar">
              {isVerifyingPassword ? (
                ...
                >
                  ...
                  <div>
                    <h3 className="text-xl font-black text-slate-900 tracking-tight">Xác thực quyền chuyển</h3>
                    <p className="text-sm text-slate-500 font-medium px-6 mt-2">Vui lòng nhập mật khẩu để phê duyệt giao dịch chuyển quỹ này.</p>
                  </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Số tiền chuyển (K)</label>
                      <div className="relative group">
                        <input
                          ...
                          placeholder="0"
                          className="w-full bg-slate-50 border-none rounded-2xl p-4 md:p-5 pr-14 text-2xl md:text-3xl font-black text-slate-900 placeholder:text-slate-200 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all"
                        />
                        <div className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-300 font-black text-lg">K</div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Từ quỹ</label>
                        <div className="relative">
                          <select
                            value={sourceFundId}
                            onChange={(e) => setSourceFundId(e.target.value)}
                            className="w-full bg-slate-50 border-none rounded-2xl p-4 font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/20 outline-none appearance-none cursor-pointer"
                          >
                            {funds.map((f) => (
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Đến quỹ</label>
                        <div className="relative">
                          <select
                            value={targetFundId}
                    ...
                    {/* Note */}
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Ghi chú (Tùy chọn)</label>
                      <input
                        type="text"
                        value={note}
                        onChange={(e) => setNote(e.target.value)}
                        placeholder="Lý do chuyển đổi..."
                        className="w-full bg-slate-50 border-none rounded-2xl p-4 font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all placeholder:text-slate-300"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex gap-4">
                    <button
                      type="button"
                      onClick={close}
                      className="flex-1 px-6 py-4 md:py-5 bg-slate-50 text-slate-600 rounded-2xl font-black hover:bg-slate-100 transition-all border border-slate-100"
                    >
                      Hủy
                    </button>
                    <button
                      ...
                      className="flex-[2] px-8 py-4 md:py-5 bg-blue-600 text-white rounded-2xl font-black hover:bg-blue-700 transition-all flex items-center justify-center gap-3 shadow-xl shadow-blue-100 disabled:opacity-50"
                    >
